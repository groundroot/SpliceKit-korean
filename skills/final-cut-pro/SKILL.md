---
name: final-cut-pro
description: Apple Final Cut Pro FCPXML format reference + 검증된 일괄 워크플로 레시피. Covers project structure, timeline creation, clip references, effects, transitions. Use when generating FCP projects, understanding FCPXML structure, or when asked to 폴더의 영상들을 파컷에 링크로 연결/프로젝트 자동 생성/타임라인에 올리기/프로젝트별 FCPXML 내보내기 (batch-import a folder of videos into FCP, one project per clip, or export every project as FCPXML).
---
...
## Recipes (검증됨 — FCP 12.3 / SpliceKit 3.3.9, 2026-08-14)

splicekit MCP가 붙어 있을 때 쓰는 실행 경로. 아래 두 레시피는 실제로 돌려서 확인한 것이므로
매번 다시 탐색하지 말고 그대로 따를 것.

### A. 폴더의 영상 → 영상당 프로젝트 1개, 타임라인에 올리기 (원본 링크)

```bash
python3 ~/.claude/skills/final-cut-pro/scripts/folder_to_fcpxml.py "<폴더>" \
  -o /tmp/import.fcpxml -e "<이벤트명>"
```

그 다음 생성된 XML 내용을 `import_fcpxml(xml=<내용>, internal=True)` 로 넘긴다.
`{"importOK": true}` 가 오면 끝. 확인은 `open_project(name=...)` → `get_timeline_clips()`.

스크립트가 알아서 하는 것: ffprobe로 소스별 해상도·프레임레이트·길이·오디오 채널을 읽어
시퀀스 포맷을 소스에 그대로 맞춤(리사이즈/리타이밍 없음), 파일명 특수문자 URL 인코딩,
`xmllint` 검증.

### B. 라이브러리의 모든 프로젝트를 FCPXML로 내보내기

`export_xml()`은 **현재 열린 프로젝트 하나만** 내보낸다. 따라서 프로젝트마다
`open_project(name=...)` → `export_xml(path=...)` 를 반복한다. 배치 툴은 없다.

### 함정 (다시 부딪히지 말 것)

- `generate_fcpxml()` MCP 툴은 gap/title/marker/transition만 만든다. **asset-clip을 못 만든다** →
  실제 영상을 얹으려면 FCPXML을 직접 써야 한다 (그게 위 스크립트).
- `import_otio(path=...)` 는 경로를 받아서 매력적이지만 브리지에 `opentimelineio`가
  없으면 실패한다. 미설치 상태면 `import_fcpxml(xml=...)` 로 문자열을 넘기는 수밖에 없다.
  (`pip install opentimelineio otio-fcpxml-adapter` 하면 경로 임포트가 열림 — 사용자 승인 필요)
- `<library>` 에 `location` 을 **적지 않으면** 현재 열린 라이브러리로 들어간다. 원하는 동작.
- `media-rep kind="original-media"` 로 임포트하면 FCP가 라이브러리 안에
  `Original Media/` 심볼릭 링크만 만든다 — 미디어 복사 없음(링크 연결). 확인: `du -sh *.fcpbundle`
- `open_project(event="...")` 는 잘 실패한다. 브리지가 project 목록의 event 필드를
  빈 문자열로 돌려주기 때문. **event는 넘기지 말고 name 부분 문자열로만** 매칭할 것.
- 같은 이름의 프로젝트가 여러 개면 `open_project` 는 내용 있는 쪽을 고르는 편이지만
  보장은 없다. 내보내기 후 `grep -c '<asset-clip'` 로 검증할 것.
- `frameDuration` 은 ffprobe `r_frame_rate` 의 **역수**다. `30000/1001` fps → `1001/30000s`.
- 프로젝트명을 파일명으로 쓸 때 `｜ ＂ … | !!` 같은 문자는 잘라내는 게 안전하다.
